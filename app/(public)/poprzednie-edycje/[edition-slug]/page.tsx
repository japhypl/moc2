...
type Props = {
  params: Promise<{ "edition-slug": string }>;
};

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { "edition-slug": slug } = await params;
  ...
  return {
    ...
    openGraph: event.og_image
      ? { images: [{ url: event.og_image }] }
      : undefined,
  };
}
...
export default async function PastEditionPage({ params }: Props) {
  const { "edition-slug": slug } = await params;
  const event = await getEventBySlug(slug);
        ...
        <Container>
          <Breadcrumbs
            items={[
              ...
              { label: "Poprzednie edycje", href: "/poprzednie-edycje" },
              { label: event.title },
            ]}
          />
          ...
          {/* Past event notice */}
          <div className="mt-6 rounded-lg bg-muted/50 px-5 py-4 text-text-muted">
